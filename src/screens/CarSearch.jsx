...
import { IconArrowLeft, IconMapPin, IconCalendar, IconCar, IconSearch } from '@tabler/icons-react';
...
const CarSearch = () => {
  ...
  const [pickupCity, setPickupCity] = useState(incomingState.pickup || 'Tirupati');
  const [dropoffCity, setDropoffCity] = useState(incomingState.dropoff || 'Rishikesh');
  const [pickupDate, setPickupDate] = useState(incomingState.date || '2025-06-12');
  const [returnDate, setReturnDate] = useState('2025-06-15');
  ...
  return (
    <div className="min-h-full bg-bgBase flex flex-col w-full relative font-sans">
      ...
      <div className="w-full h-[350px] relative overflow-hidden flex flex-col justify-between">
        <img 
          src="/assets/explore_adventure_1778623926947.png" 
        ...
        <div className="relative z-10 px-6 md:px-12 py-6 flex items-center gap-4">
          ...
          <div>
            <h1 className="font-serif text-[24px] text-white leading-tight font-bold">Premium Cabs</h1>
        ...
        <div className="relative z-10 px-6 md:px-12 pb-12">
          <h2 className="font-serif text-[36px] md:text-[48px] text-white leading-[1.1] mb-3 font-bold">
            Travel in <span className="text-transparent bg-clip-text bg-gradient-to-r from-goldAmber to-yellow-200">comfort & luxury</span>
          </h2>
          <p className="text-zinc-300 text-[16px] max-w-xl">
            Book verified outstation cabs, local rentals, and premium airport transfers for your journey.
          </p>
        </div>
      </div>
          ...
          <div className="flex flex-col lg:flex-row gap-3">
            ...
            <div className="flex-1 bg-white/5 hover:bg-white/10 transition-colors rounded-xl p-3 flex items-center gap-3 border border-transparent focus-within:border-goldAmber/50">
              ...
              <div className="flex-1">
                <label className="block text-zinc-400 text-[10px] uppercase font-bold tracking-wider mb-0.5">Pick-up</label>
                <input type="text" placeholder="City or Airport" className="w-full bg-transparent text-white text-[15px] font-medium outline-none placeholder:text-zinc-600" value={pickupCity} onChange={(e) => setPickupCity(e.target.value)} />
              </div>
            </div>
            ...
            <div className="flex-1 bg-white/5 hover:bg-white/10 transition-colors rounded-xl p-3 flex items-center gap-3 border border-transparent focus-within:border-goldAmber/50">
              <IconMapPin size={24} className="text-goldAmber shrink-0" stroke={1.5} />
              <div className="flex-1">
                <label className="block text-zinc-400 text-[10px] uppercase font-bold tracking-wider mb-0.5">Drop-off</label>
                <input type="text" placeholder="City, Airport or Local" className="w-full bg-transparent text-white text-[15px] font-medium outline-none placeholder:text-zinc-600" value={dropoffCity} onChange={(e) => setDropoffCity(e.target.value)} />
              </div>
            </div>
            ...
            <div className="flex-[0.8] bg-white/5 hover:bg-white/10 transition-colors rounded-xl p-3 flex items-center gap-3 border border-transparent focus-within:border-goldAmber/50">
              ...
              <div className="flex-1">
                <label className="block text-zinc-400 text-[10px] uppercase font-bold tracking-wider mb-0.5">Pick-up Date</label>
                <input type="date" className="w-full bg-transparent text-white text-[15px] font-medium outline-none cursor-pointer [color-scheme:dark]" value={pickupDate} onChange={(e) => setPickupDate(e.target.value)} />
              </div>
            </div>
            ...
            <button 
              onClick={() => navigate('/car-results', { state: { pickupCity, dropoffCity, date: pickupDate } })}
      ...
      <div className="px-6 md:px-12 flex-1 pb-12">
        <h3 className="text-white font-serif text-[20px] mb-6">Popular Routes</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { from: 'Mumbai', to: 'Shirdi' }
             ...
             <div key={i} onClick={() => { setPickupCity(route.from); setDropoffCity(route.to); }} className="bg-surfaceElevated hover:bg-white/10 border border-borderDefault hover:border-goldAmber/50 transition-all cursor-pointer rounded-xl p-4 flex flex-col items-center justify-center text-center gap-2">
                <IconCar size={28} className="text-goldAmber" stroke={1.5}/>
                <span className="text-white text-[14px] font-medium">{route.from} ➔ {route.to}</span>
             </div>
          ))}
        </div>
      </div>
